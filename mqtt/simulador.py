...
def buscar_estacoes(token):
    ...
    try:
        estacoes = requests.get(f'{API_URL}/estacoes', headers=headers, timeout=10).json()
        parametros = requests.get(f'{API_URL}/parametros', headers=headers, timeout=10).json()
...
def gerar_valor(nome_tipo):
    ...
    if 'temperatura' in nome:
        if extremo:
            return round(random.choice([random.uniform(-15.0, -5.0), random.uniform(45.0, 55.0)]), 2)
        return round(random.uniform(18.0, 35.0), 2)

    elif 'umidade' in nome:
        if extremo:
            return round(random.choice([random.uniform(0.0, 10.0), random.uniform(98.0, 100.0)]), 2)
        return round(random.uniform(40.0, 80.0), 2)

    elif 'pressao' in nome:
        if extremo:
            return round(random.choice([random.uniform(850.0, 920.0), random.uniform(1045.0, 1080.0)]), 2)
...
def publicar(cliente, estacao, parametro, valor):
    payload = {
        'uid': estacao.get('uid') or f'EST{estacao["id"]}',
        'nome_estacao': estacao['nome'],
        'nome_parametro': parametro.get('nome_tipo', 'Desconhecido'),
        'id_parametro': parametro['id'],
        'valor': valor,
        'timestamp': datetime.now().isoformat()
    }
    cliente.publish(TOPICO, json.dumps(payload))
    print(f'[MQTT] {estacao["nome"]} | {parametro.get("nome_tipo", "?")} : {valor}')
...
while True:
    ...
    ativas = [e for e in estacoes if isinstance(e, dict) and e.get('ativo')]

    print(f'[API] {len(ativas)} estacoes ativas encontradas')
    ...
    for estacao in ativas:
        ...
        for param in params_estacao:
            valor = gerar_valor(param.get('nome_tipo', ''))
            publicar(con, estacao, param, valor)
